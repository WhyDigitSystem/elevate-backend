...
public interface TrialBalanceRepo extends JpaRepository<TrialBalanceVO, Long> {
	...
	@Query(nativeQuery = true,value = "WITH params AS (\r\n"
			...
			+ "    JOIN coa c ON b.accountcode = c.parentcode \r\n"
			+ "              AND c.orgid = d.orgid \r\n"
			...
			+ ") \r\n"
			+ "AND (h.coacode, h.coa) NOT IN (\r\n"
			+ "    SELECT b.accountcode, b.accountname \r\n"
			+ "    FROM monthlyprocess a, monthlyprocessdetails b \r\n"
			+ "    WHERE a.monthlyprocessid = b.monthlyprocessid \r\n"
			+ "    AND a.year = ?3\r\n"
			+ "    AND a.month = ?4\r\n"
			+ "    AND a.orgid = ?1\r\n"
			+ "    AND a.clientcode = ?2 \r\n"
			+ "    AND a.maingroup = ?6 \r\n"
			+ "    AND a.subgroupcode = ?7\r\n"
			...
			+ " from monthlyprocessdetails a where a.orgid=?1 and a.clientcode=?2 and a.month=?4 and a.year=?3 and a.maingroup=?6  and a.groupcode=?7")
