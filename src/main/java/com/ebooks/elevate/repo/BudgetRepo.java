...
public interface BudgetRepo extends JpaRepository<BudgetVO, Long> {
	...
	@Query(nativeQuery = true, value = "select accountname,quater,sum(amount) from previousyearactual where orgid=?1 and maingroup=?4 and clientcode=?3 and subgroup=?5 and year = ?2\r\n"
			+ "group by  accountname,quater\r\n"
			+ "union\r\n"
			+ "select accountname,'YTD' as quater,sum(amount) from previousyearactual where orgid=?1 and maingroup=?4 and clientcode=?3 and subgroup=?5 and year = ?2\r\n"
			+ "and  monthsequence <= (\r\n"
			+ "			          SELECT MAX(monthsequence)\r\n"
			+ "			         FROM previousyearactual\r\n"
			+ "			          WHERE year = ?2\r\n"
